...
# Calculate compatibility scores for each student against all other students
compatibility_scores = {}
for i, student1 in enumerate(students_data):
    compatibility_scores[student1['student_id']] = {}
    for j, student2 in enumerate(students_data):
        if i != j:
            score = calculate_compatibility_score(student1, student2, weights)
            compatibility_scores[student1['student_id']][student2['student_id']] = score

# Output compatibility scores
for student_id, scores in compatibility_scores.items():
    print(f"Student {student_id}:")
    for other_student_id, score in scores.items():
        print(f"    - Student {other_student_id}: Compatibility Score {score}")
